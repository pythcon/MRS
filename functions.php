...
function submitRequest($user, $location, $area, $category, $description, $files){
    ...
    try {
        ...
        $sql = "INSERT INTO requests(rid, user, location, area, category, description, files) VALUES ($rid, '$user', '$location', '$area' '$category', '$description', '$files')";
        $q = $db->prepare($sql);
        
        echo"
        <script>
            alert(\"$rows, $user, $location, $area, $category, $description, $files\");
        </script>";
